...
static func get_four_directions_vector(diagonal_allowed: bool) -> Vector2:
...
static func get_random_vector(snap: bool = false, min_x: float = -1.0, max_x:float = 1.0, min_y: float= -1.0, max_y: float = 1.0):
	var vector: Vector2 = Vector2(randf_range(min_x, max_x) , randf_range(min_y,max_y))
	if snap:
		vector.x = round(vector.x)
		vector.y = round(vector.y)
	return vector
static func get_random_vector_id() -> Vector2:
...
static func convert_vec2_to_facing_int(dir:Vector2)->int:
	if dir.y <0:
		...
		return Facing.DOWN
static func convert_facing_int_vec2(n:int)-> Vector2:
	match n:
		Facing.UP:
			return Vector2.UP
		Facing.DOWN:
